...
export function DJExecutionTimeline({ execution }: DJExecutionTimelineProps) {
  const steps = STEP_ICONS.map((s) => ({
    ...s,
    value: execution[s.key as keyof DJExecution] || "",
  }));
      ...
      <div className="space-y-0">
        {steps.map((step, i) => (
          <div key={step.key} className="relative flex gap-4 pb-6 last:pb-0">
            {i < steps.length - 1 && (
              <div className="absolute left-[15px] top-7 bottom-0 w-px bg-zinc-700" />
            )}
            <div className="relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-zinc-700 bg-zinc-900">
