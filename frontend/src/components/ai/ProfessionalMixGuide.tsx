...
function difficultyBadge(difficulty: string) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    Easy: {
      label: "Easy Mix",
      cls: "border-success/20 bg-success/5 text-success/80",
    },
    Medium: {
      label: "Moderate Mix",
      cls: "border-yellow-400/30 bg-yellow-400/10 text-yellow-400",
    },
    Hard: {
      label: "Hard Mix",
      cls: "border-orange-400/30 bg-orange-400/10 text-orange-400",
    },
    Expert: {
      label: "Expert Mix",
...
function matchBadge(overallRating: string) {
  ...
  return (
    map[overallRating] ?? { label: "AI Assisted", cls: "text-text-secondary" }
  );
}
...
function ScoreRing({ score }: { score: number }) {
  ...
  return (
    <div className="relative inline-flex items-center justify-center shrink-0">
      ...
      <span
        className="absolute text-base font-bold tracking-tight"
        style={{ color }}
      >
        {score}
      </span>
...
export function ProfessionalMixGuide({
  recommendation: r,
  compatibility,
}: ProfessionalMixGuideProps) {
  ...
  return (
    <section className="rounded-xl border border-zinc-800 bg-card/75 p-4 shadow-[0_20px_60px_-35px_rgba(0,0,0,0.9)] backdrop-blur md:p-5">
      ...
      <header className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <span
          className={`rounded-full border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${diff.cls}`}
        >
          {diff.label}
        </span>
      </header>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-3">
        ...
        <div className="flex flex-1 flex-wrap items-center gap-x-4 gap-y-1 min-w-45">
          <MiniStat
            label="Risk"
            value={r.risk_level}
            valueClass={riskColor(r.risk_level)}
          />
          <MiniStat
            label="Transition"
            value={r.recommended_transition_length}
          />
          <MiniStat
            label="Quality"
            value={r.transition_quality}
            valueClass={
              r.transition_quality.toLowerCase() === "high"
                ? "text-success"
                : r.transition_quality.toLowerCase() === "medium"
                  ? "text-yellow-400"
                  : "text-danger"
            }
          />
      ...
      {/* ── SUMMARY ── */}
      {r.summary && (
        <div className="my-4 rounded-lg border border-primary/10 bg-primary/2 px-4 py-3 text-center">
      ...
      {/* ── EXECUTION ── */}
      <div className="mb-3">
        <h4 className="mb-1.5 text-[9px] font-semibold uppercase tracking-[0.15em] text-text-secondary/60">
          Execution
        </h4>
        <div className="grid gap-2 sm:grid-cols-3">
          <ExecutionCard
            label="Before"
            value={r.mix_strategy.before_transition}
          />
          <ExecutionCard
            label="During"
            value={r.mix_strategy.during_transition}
          />
          <ExecutionCard
            label="After"
            value={r.mix_strategy.after_transition}
          />
        </div>
      </div>
      ...
      >
        ...
        <Section title="Tempo Analysis">
          ...
          <CompactRow
            ...
            value={r.tempo_analysis.recommendation}
          />
        </Section>
        ...
        <Section title="Energy Analysis">
          ...
          <CompactRow
            label="Recommendation"
            value={r.energy_analysis.recommendation}
          />
        </Section>
        ...
        <Section title="Compatibility">
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 text-xs">
            ...
            <span className="font-semibold text-text">
              {compatibility.compatibility_score.toFixed(1)}%
            </span>
            <span className={`text-[10px] font-medium ${match.cls}`}>
              ({match.label})
            </span>
            <span className="text-text-secondary/30">|</span>
            <span className="text-text-secondary/50">LLM:</span>
            <span className="text-text/80">
              {r.compatibility_analysis.score}
            </span>
          </div>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                {djExecEntries.map(([key, value]) => (
                  <div key={key} className="flex gap-1.5 text-xs">
                    <span className="shrink-0 text-text-secondary/40 capitalize">
                      {key.replace(/_/g, " ")}:
                    </span>
        ...
        {r.club_tip && (
          <>
            <Section title="Club Tip">
              <div className="flex items-start gap-1.5 rounded-lg border border-success/15 bg-success/3 px-2.5 py-1.5">
            ...
            <Section title={`Risks (${r.risks.length})`}>
              <div className="flex flex-wrap gap-1.5">
                {r.risks.slice(0, 2).map((risk, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-1 rounded-full border border-danger/15 bg-danger/4 px-2 py-0.5 text-[10px] text-danger/80"
                  >
                    {risk}
                  </span>
                ))}
...
function MiniStat({
  ...
  return (
    <div className="leading-tight">
      <p className="text-[9px] uppercase tracking-[0.13em] text-text-secondary/50">
        {label}
      </p>
      <p className={`text-xs font-semibold ${valueClass ?? "text-text"}`}>
        {value}
      </p>
    </div>
  );
...
function ConfidenceBar({ confidence }: { confidence: number }) {
  const color =
    confidence >= 80 ? "#21d969" : confidence >= 50 ? "#facc15" : "#ff5b6e";
  return (
    <div className="flex items-center gap-2">
...
const ExecutionCard = memo(function ExecutionCard({
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/30 px-2.5 py-2">
      <p className="text-[9px] font-semibold uppercase tracking-[0.15em] text-text-secondary/60">
        {label}
      </p>
...
function Section({
  ...
  return (
    <div>
      <h5 className="text-[9px] font-semibold uppercase tracking-[0.15em] text-text-secondary/40 mb-1.5">
        {title}
      </h5>
      {children}
    </div>
    ...
    <div className="space-y-0" style={{ maxHeight: 200, overflowY: "auto" }}>
      {entries.map(([key, action], i) => (
        <div key={key} className="relative flex gap-3 pb-3 last:pb-0">
          {i < entries.length - 1 && (
            <div className="absolute left-2.75 top-4 bottom-0 w-px bg-zinc-700" />
          )}
          <div className="relative z-10 mt-1 h-2.5 w-2.5 shrink-0 rounded-full border-2 border-primary bg-zinc-900" />
...
function ModelFooter({
  ...
  if (fallbackOccurred != null)
    info.push(fallbackOccurred ? "Fallback: Yes" : "Fallback: No");
