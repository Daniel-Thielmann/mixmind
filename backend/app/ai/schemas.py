"""Pydantic schemas for DJ assistant prompts and responses."""

from pydantic import BaseModel, ConfigDict, Field


class TempoAnalysis(BaseModel):
    """BPM-based analysis for the track pair."""
...
class EnergyAnalysis(BaseModel):
    """RMS-energy comparison for the track pair."""
...
class CompatibilityAnalysis(BaseModel):
    """Backend compatibility score interpreted for the DJ."""
...
class MixStrategy(BaseModel):
    """Step-by-step mixing approach across three phases."""
...
class DJExecution(BaseModel):
    """Specific techniques the DJ should apply."""
...
class AIRecommendationResponse(BaseModel):
    """Validated DJ assistant response returned to the API."""

    model_config = ConfigDict(extra="forbid")

    ai_provider: str | None = Field(
        default=None,
        description="LLM provider name (e.g. openrouter).",
    )
    ...
    club_tip: str = Field(
        default="",
        description="Practical club or performance tip.",
    )
    professional_notes: str = Field(
        default="",
        description="Additional professional observations.",
    )
    risks: list[str] = Field(
        default_factory=list,
        description="Potential risks or failure modes.",
    )
    best_use_case: str = Field(
        default="",
        description="Ideal set context for this track pair.",
    )
    ...
    # -- New Professional Fields (M3-M8) --

    mix_difficulty: str = Field(
        default="Medium",
        description=(
            "Calculated mix difficulty (Very Easy / Easy / Medium / Hard / Expert)."
        ),
    )
    recommended_transition_length: str = Field(
        default="16 bars",
        description=(
            "Recommended transition length (8 bars / 16 bars / 32 bars / 64 bars)."
        ),
    )
