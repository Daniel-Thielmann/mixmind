...
def log_memory_detail(tag: str, extra: str = "") -> None:
    ...
    uss_val = getattr(pf, "uss", None)
    uss = (uss_val / (1024 * 1024)) if uss_val else 0.0
    logger.info(
...
def log_library_versions() -> None:
    ...
    try:
        ...
        logger.info("  audioread: %s", getattr(audioread, "__version__", "unknown"))
    except Exception:
...
def _log_librosa_backends() -> None:
    ...
    try:
        ...
        logger.info(
            "  audioread version: %s", getattr(audioread, "__version__", "unknown")
        )
    except Exception:
        logger.info("  audioread: not available")
